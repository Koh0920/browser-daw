...
const AudioEditor = ({ track }: AudioEditorProps) => {
  ...
  const { currentProject, splitAudioClip, trimAudioClip } = useProjectStore()
  ...
  const drawAudioClip = (ctx: CanvasRenderingContext2D, clip: AudioClip, isSelected: boolean) => {
    const x = clip.startTime * zoom
    const width = clip.duration * zoom

    // Draw clip background
    ctx.fillStyle = isSelected ? "rgba(255, 80, 80, 0.5)" : "rgba(255, 80, 80, 0.3)"
    ctx.fillRect(x, 0, width, canvasHeight)

    // Draw clip border
    ctx.strokeStyle = isSelected ? "rgba(255, 80, 80, 1)" : "rgba(255, 80, 80, 0.8)"
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (dragAction === "move") {
      ...
      trimAudioClip(track.id, selectedClip.id, newStartTime, selectedClip.duration)
    } else if (dragAction === "trim-start") {
      // Trim the start of the clip
      const newStartTime = Math.max(0, selectedClip.startTime + dx)
      const newDuration = Math.max(0.1, selectedClip.duration - dx)
      trimAudioClip(track.id, selectedClip.id, newStartTime, newDuration)
    } else if (dragAction === "trim-end") {
      // Trim the end of the clip
      const newDuration = Math.max(0.1, selectedClip.duration + dx)
      trimAudioClip(track.id, selectedClip.id, selectedClip.startTime, newDuration)
    }
  ...
  return (
    <div className="flex flex-col h-full" ref={containerRef}>
      <div className="flex items-center gap-2 p-2 border-b border-border">
        <Button variant="outline" size="sm" onClick={handleSplitClip} disabled={!selectedClip}>
        ...
        {selectedClip && <div className="text-sm ml-4">Selected: {selectedClip.name}</div>}
      </div>
